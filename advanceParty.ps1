# advanceParty - setup fireBase for fireMissions - because an advance party don't stop!

# Create an operations directory and hide it
$opsDirectory = "C:\RSAT"
...
# Check to see if our payload is already downloaded. Note we need to
# specify our TLS version to 1.2 to get the download to negoatiate
# a secure channel.
...
# Check to see if our config is already downloaded. Note we need to
# specify our TLS version to 1.2 to get the download to negoatiate
# a secure channel.
...
# Load the newly unzipped directory path into a variable
# and set the destiation path to extract the config
...
# Modify the contents of the configuration files to change the hardcoded variable
# in downloaded configs to the hostname of the machine
