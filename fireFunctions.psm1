...
function fireMission ($outputPane,$progressBar) {
...

  {
    ...
    $cmdstring = "invoke-command -computername $client -scriptblock {write-host 'I am' $client ; & ‘$opsDir\xmr-stak-win64-2.7.1\xmr-stak.exe’ -c '$opsDir\xmr-stak-win64-2.7.1\config.txt' -C '$opsDir\xmr-stak-win64-2.7.1\pools.txt' --cpu '$opsDir\xmr-stak-win64-2.7.1\cpu.txt' --amd '$opsDir\xmr-stak-win64-2.7.1\amd.txt' --nvidia '$opsDir\xmr-stak-win64-2.7.1\nvidia.txt'}"
    $scriptBlock = [scriptblock]::Create($cmdstring)
    Start-Process powershell -ArgumentList "-command  $Scriptblock"
